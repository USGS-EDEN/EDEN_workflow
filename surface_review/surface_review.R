...
depth_surf <- list.files("./surfaces", "^[0-9]{4}_q[1-4]_depth.nc$")
...
for (j in 1:dim(gages)[1]) {
  ...
  if (!dir.exists(paste0("./surf_edendb_graphs/", substr(file_surf[i], 1, 7)))) dir.create(paste0("./surf_edendb_graphs/", substr(file_surf[i], 1, 7)))
  jpeg(paste0("./surf_edendb_graphs/", substr(file_surf[i], 1, 7), "/", gages$station_name_web[j], ".jpg"), width = 1200, height = 800, units = "px", pointsize = 12, quality = 100, bg = "white", type = "quartz")
  par(mar = c(5, 4, 4, 5) + .1)
  ...
  dev.off()
  if (!dir.exists(paste0("./surf_edendb_thumbs/", substr(file_surf[i], 1, 7)))) dir.create(paste0("./surf_edendb_thumbs/", substr(file_surf[i], 1, 7)))
  jpeg(paste0("./surf_edendb_thumbs/", substr(file_surf[i], 1, 7), "/", gages$station_name_web[j], ".jpg"), width = 240, height = 160, units = "px", pointsize = 12, quality = 100, bg = "white", type = "quartz")
  par(mar = c(0, 0, 0, 0))
...
for (j in 1:length(time)) {
  if (!dir.exists(paste0("./wl_contours/", substr(file_surf[i], 1, 7)))) dir.create(paste0("./wl_contours/", substr(file_surf[i], 1, 7)))
  png(paste0("./wl_contours/", substr(file_surf[i], 1, 7), "/day", sprintf("%04d", j - 1), ".png"), width = 1228, height = 1724, units = "px", pointsize = 12, bg = "transparent", type = "quartz")
  par(mar = c(0, 0, 0, 0))
  image(x, y, stage[, , j], col = stage.col, breaks = seq(r1[1], r1[2], length.out = 82), axes = F, asp = 1, xlab = "", ylab = "")
  text(x[1], y[10], as.Date(time[j]), pos = 4)
  # 3cm contours "by = 3"
  contour(x, y, stage[, , j], levels = seq(r1[1], r1[2], by = 3), lwd = c(1, 0.5), add = T)
  dev.off()

  if (!dir.exists(paste0("./depth_contours/", substr(file_surf[i], 1, 7)))) dir.create(paste0("./depth_contours/", substr(file_surf[i], 1, 7)))
  png(paste0("./depth_contours/", substr(file_surf[i], 1, 7), "/day", sprintf("%04d", j - 1), ".png"), width = 1228, height = 1724, units = "px", pointsize = 12, bg = "transparent", type = "quartz")
  par(mar = c(0, 0, 0, 0))
  image(x, y, depth[, , j], col = depth.col, breaks = seq(r2[1], r2[2], length.out = 82), axes = F, asp = 1, xlab = "", ylab = "")
  text(x[1], y[10], as.Date(time[j]), pos = 4)
  contour(x, y, depth[, , j], levels = seq(r2[1], r2[2], by = 3), lwd = c(1, 0.5), add = T)
  dev.off()
}
}
setwd("..")
