...
for (d in num_days) {
  ...
  end <- strptime(paste(days[length(days)], "23:54:00"), "%Y-%m-%d %H:%M:%S", tz="EST")
  ...
  for (i in 1:length(days)) {
    enp_files <- c(enp_files, list.files(paste0(dir, "/enp/"), format(days[i], "%Y%m%d")))
    sfwmd_files <- c(sfwmd_files, list.files(paste0(dir, "/sfwmd/"), format(days[i], "%Y%m%d")))
  }
  ...
  for (i in 1:length(usgs_gages$station_name_web)) {
    ...
    if (inherits(tmp, "try-error")) { report <- paste0(report, "USGS data _NOT_ downloaded for ", usgs_gages$station_name_web[i], "\n") } else {
  ...
  for (i in 1:length(enp_files)) {
    # Read in ENP data
    enp_tmp <- read.table(paste0(dir, "/enp/", enp_files[i]), colClasses = c("NULL", "character", "character", "character"))
    # Create timestamp
  ...
  for (i in 1:length(sfwmd_files)) {
    # Read in SFWMD data
    sfwmd_tmp <- read.fwf(paste0(dir, "/sfwmd/", sfwmd_files[i]), widths = c(11, 17, 13, 13, 10, 2), strip.white = T, colClasses = c("character", "character", "character", "character", "numeric", "character"))
    # Create timestamp
