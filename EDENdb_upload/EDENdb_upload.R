...
# install.packages(c("RMySQL", "RCurl"))
library (RMySQL)
library (RCurl)
...
if (inherits(err, "try-error")) { report <- "ISOutput_Run.txt input file not downloaded.\n"
  ...
  # Retain all timestamps from within last n days
  ...
  #for quarters:
  #gages <- dbGetQuery(con, "select station_name, station_name_web, agency_acronym as agency, utm_easting, utm_northing, dry_elevation, convert_to_navd88_feet as conv from station, agency, station_datum where station.database_agency_id = agency.agency_id and station.station_id = station_datum.station_id and edenmaster_start <= '2021q1' and edenmaster_end >= '2021q1' group by station_name order by agency, station_name_web")
