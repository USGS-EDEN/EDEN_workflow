...
for (i in 1:dim(gages)[1]) {
  ...
  query <- paste0(query, " from stage where datetime >= 20190930000000 and datetime < 20200102000000 order by datetime")
  ...
  if (any(!is.na(ts$st))) {
    ...
    plot(ts$datetime, ts$st, type = "n", lwd = 2, ylim = yran, main = paste("Late 2019q4 for", gages$station_name_web[i]), xlab = "Date", ylab = "Stage (ft.)")
