...
for (i in 1:dim(gages)[1]) {
  ...
  query <- paste0(query, " from stage where datetime >= 20200617000000 and datetime < 20201014000000 order by datetime")
  ...
  if (any(!is.na(ts$st))) {
    ...
    plot(ts$datetime, ts$st, type = "n", ylim = yran, main = paste("2020q3 for", gages$station_name_web[i]), xlab = "Date", ylab = "Stage (ft.)")
    abline(v = ts$datetime[ts$datetime == "2020-06-30 23:00:00"], lwd = 3, col = "darkred")
    abline(v = ts$datetime[ts$datetime == "2020-10-01 01:00:00"], lwd = 3, col = "darkred")
    ...
    legend("topleft", c(paste(gages$station_name_web[i], "measured"), paste(gages$station_name_web[i], "estimated"), paste("P1:", pred$station_name_web[1]), paste("P2:", pred$station_name_web[2]), paste("P3:", pred$station_name_web[3])), pch = 16, col = c("black", "orange", "blue", "red", "darkgreen"))
    dev.off()
  }
}
setwd("..")
