...
for (i in 1:dim(gages)[1]) {
  ...
  query <- paste0(query, " from stage where datetime >= 20191218000000 and datetime < 20200415000000 order by datetime")
  ...
  if (any(!is.na(ts$st))) {
    ...
    abline(v = ts$datetime[ts$datetime == "2019-12-31 23:00:00"], lwd = 3, col = "darkred")
    abline(v = ts$datetime[ts$datetime == "2020-04-01 01:00:00"], lwd = 3, col = "darkred")
