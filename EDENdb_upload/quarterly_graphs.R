...
for (i in 1:dim(gages)[1]) {
  ...
  query <- paste0(query, " from stage where datetime >= 20200318000000 and datetime < 20200715000000 order by datetime")
  ...
  if (any(!is.na(ts$st))) {
    ...
    plot(ts$datetime, ts$st, type = "n", lwd = 2, ylim = yran, main = paste("2020q2 for", gages$station_name_web[i]), xlab = "Date", ylab = "Stage (ft.)")
    abline(v = ts$datetime[ts$datetime == "2020-03-31 23:00:00"], lwd = 3, col = "darkred")
    abline(v = ts$datetime[ts$datetime == "2020-07-01 01:00:00"], lwd = 3, col = "darkred")
