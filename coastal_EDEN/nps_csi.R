...
for (j in 2:dim(db)[1]) {
  ...
  err <- try (ftpUpload(paste0("./csi/", db$COLUMN_NAME[j], "_input.csv"), paste0("ftp://ftpint.usgs.gov/pub/er/fl/st.petersburg/eden/nps_csi/csi_values/", db$COLUMN_NAME[j], "_input.csv")))
  q <- paste0(query, ", avg(", db$COLUMN_NAME[j], ") as ", db$COLUMN_NAME[j], " from nps_salinity group by Year, Month")
  sal <- dbGetQuery(con, q)
  csi <- CSIcalc(sal)
  CSIwrite(csi, "./csi")
  err <- try (ftpUpload(paste0("./csi/", db$COLUMN_NAME[j], ".csv"), paste0("ftp://ftpint.usgs.gov/pub/er/fl/st.petersburg/eden/nps_csi/csi_values/", strsplit(db$COLUMN_NAME[j], "_")[[1]][1], ".csv"), .opts = list(forbid.reuse = 1)))
}
